...
class VtexResolver implements Resolver
{
    ...
    private function breakOnItems (array $orders) : array 
    {
        ...
        foreach ($orders as $order) {
            $marketingDataAux = [];
            foreach(array_keys($order['marketingData']) as $marketingDataKey){
                $marketingDataAux['marketingData_'.$marketingDataKey] = $order['marketingData'][$marketingDataKey];
            }
            foreach ($order['items'] as $item) {
                $itemAux = [];
                foreach(array_keys($item) as $key){
                    $itemAux['item_'.$key] = $item[$key];
                }
                $record = array_merge($itemAux, $order, $marketingDataAux);
                unset($record['items']);
                unset($record['marketingData']);
                $items[] = $record;
            }
        }
        return $items;
